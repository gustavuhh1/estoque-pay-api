...
| #46 | CRUD de Categorias e Vínculos N:N | OPEN | ✅ **Feito nesta branch (aguardando merge)** | `POST /categoria` cria categoria na loja ativa, com `produto_ids` opcional pra já vincular produtos existentes (RF11.5). `GET /categoria` lista categorias com os produtos vinculados (id+nome). `PATCH /categoria/:id` edita só o nome. O vínculo N:N é sempre incremental: `POST /categoria/:id/produtos` adiciona produtos (Prisma `connect`, sem afetar os já vinculados) e `DELETE /categoria/:id/produtos` remove (Prisma `disconnect`, idempotente, nunca apaga o `Produto`) — decisão tomada com o usuário depois de discutir os riscos de um `PATCH` com `set` (substituir a lista inteira): concorrência (last-write-wins entre dois editores) e a necessidade de o cliente conhecer a lista completa só para mudar um item. `DELETE /categoria/:id` sempre `204`: RN02/RN08 — exclusão desimpedida mesmo com produtos vinculados, o cascade da FK do join table `_CategoriaToProduto` remove só o vínculo, nunca o `Produto`. Nome único por loja (409 `CATEGORIA_NOME_ALREADY_IN_USE`); `produto_id` de outra loja ou inexistente (404 `PRODUTO_NAO_ENCONTRADO`); CASHIER bloqueado (403 `ROLE_CANNOT_MANAGE_CATEGORIAS`). |
...
**Nota #46**: não exigiu migração nova — a tabela `categorias` e o join table implícito `_CategoriaToProduto` (com `ON DELETE CASCADE` nas duas FKs) já existiam desde `20260814180326_multi_roles`. Corrigido também um bug de limpeza entre testes (`vitest.setup.ts` não apagava `categorias` antes de `estabelecimentos`, quebrando a suíte inteira por violação de FK). 176/176 testes passando, `tsc --noEmit` limpo, coleção Postman validada via Newman (pasta `Categoria`, 22 requests / 38 assertions, e coleção inteira sem regressão). Falta: abrir/mesclar o PR (`feature/categorias-crud` → `development`, [PR #73](https://github.com/gustavuhh1/estoque-pay-api/pull/73)).
...
- **Pronto, pendente de merge**: #46 (CRUD de Categorias + vínculo N:N incremental com Produto via connect/disconnect) — implementado em `feature/categorias-crud` ([PR #73](https://github.com/gustavuhh1/estoque-pay-api/pull/73)), com 176/176 testes passando e `tsc --noEmit` limpo.
