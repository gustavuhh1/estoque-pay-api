...
**Nota #46**: não exigiu migração nova — a tabela `categorias` e o join table implícito `_CategoriaToProduto` (com `ON DELETE CASCADE` nas duas FKs) já existiam desde `20260814180326_multi_roles`. Implementação já mesclada em `development` via PR #73.
...
| Issue | Título | Status GitHub | Status Real | Observação |
| --- | --- | --- | --- | --- |
| #50 | Busca, Carrinho e Cálculo Automático | OPEN | ✅ **Feito na branch `feature/pagamento-abacatepay-gateway` (aguardando merge)** | `GET /venda/buscar?termo=` casa EXATO no código de barras e PARCIAL no nome — **produto inativo não aparece**, ao contrário do `GET /produto` (listagem de gestão). `POST /venda/calcular` precifica o carrinho e **não grava nada** (RN01): nem Venda, nem ItemVenda, nem baixa de estoque — o carrinho vive no cliente até o pagamento. Preço sempre relido do banco, nunca aceito do cliente. Subtotal arredondado em 2 casas antes de somar e total acumulado em `Decimal`, senão fracionado (0.333 kg) propagaria dízima. Produto inativo é recusado mesmo informando o id (`PRODUTO_INATIVO`), senão a trava da busca seria contornável. Sem bloqueio por cargo: vender é o trabalho do Caixa. |
| #51 | Pagamento Manual (Dinheiro e Cartão) | OPEN | ✅ **Feito na branch `feature/pagamento-abacatepay-gateway` (aguardando merge)** | `POST /pagamento` aceita `DINHEIRO`, `DEBITO` e `CREDITO`. **`PIX_ABACATEPAY` é recusado de propósito**: não é confirmação manual do caixa, nasce PENDENTE e só vira PAGO pelo webhook/polling — aceitar aqui deixaria marcar como pago um Pix que nunca caiu. Exige **turno aberto** (`409 TURNO_FECHADO`), fechando o elo com a #54. Troco calculado com `Decimal` (50 − 49.9 dá 0.09999999999999432 em float). **RNF01**: Venda + ItemVenda + baixa de estoque + auditoria numa única `` — se a baixa de qualquer item estourar o saldo, a venda inteira volta atrás, inclusive a baixa dos itens anteriores. Reusa o `registrarMovimentacao(..., tx)` da PR #76, gerando `MovimentacaoEstoque` com motivo `VENDA` (RN05). Sem bloqueio por cargo. |
| #52 | Pix Automático (AbacatePay) e Split | OPEN | 🟡 **Parcial na branch `feature/pagamento-abacatepay-gateway`** | **Feito**: `IPagamentoGateway` + `AbacatePayGateway` (POST `/transparents/create` em `https://api.abacatepay.com/v2`, `Authorization: Bearer`, conversão reais→centavos) + `InMemoryPagamentoGateway`. **RNF02 cumprida**: `POST /pagamento/webhook/abacatepay` valida HMAC-SHA256 do corpo cru em base64 (header `X-Webhook-Signature`) com comparação em tempo constante, e devolve 401 quando inválida. É a **primeira rota pública** da API (sem `requireAuth`/`requireTenant`), com parser de corpo cru isolado no escopo dela e a verificação num hook `preValidation` — antes da validação de schema, para corpo malformado sair como 401 e não 400. **Falta**: (1) a rota que cria a cobrança e grava `Venda(PENDENTE)`, e (2) a consolidação no webhook — ambas dependem de #50/#51. (3) **O split de R$ 0,80 NÃO está implementado**: a doc pública do `/transparents/create` não expõe campo para o lojista declarar split (`platformFee` aparece só na resposta), então está marcado com TODO num ponto só, a confirmar com a AbacatePay. |
| #53 | Cancelamento de Venda e Devolução ao Estoque | OPEN | ✅ **Feito na branch `feature/pagamento-abacatepay-gateway` (aguardando merge)** | `POST /venda/:venda_id/cancelar`. **RN02**: CASHIER bloqueado com `403 ROLE_CANNOT_CANCEL_VENDA` — é a **única rota de venda restrita por cargo**, porque desfaz dinheiro e mexe em estoque (buscar, calcular e pagar o Caixa faz o dia inteiro). **RN03**: devolve cada item gerando `MovimentacaoEstoque` de ENTRADA com motivo `ESTORNO_VENDA`, na mesma transação da mudança de status — e **sem apagar a SAIDA original**, então o histórico guarda as duas pontas em vez de a venda sumir como se nunca tivesse existido. Só venda **PAGA** pode ser cancelada: Pix PENDENTE abandonado nunca baixou estoque, e devolver criaria saldo do nada (`409 VENDA_NAO_PAGA`). Cancelar duas vezes é barrado (`409 VENDA_JA_CANCELADA`), senão o produto voltaria em dobro. **Não exige turno aberto**, de propósito: um gestor precisa poder corrigir erro do dia anterior antes de abrir o caixa. |
| #54 | Abertura e Fechamento de Caixa | OPEN | ✅ **Feito na branch `feature/caixa-turno` (aguardando merge)** | `POST /caixa/turno` abre, `GET /caixa/turno/atual` consulta e `PATCH /caixa/turno/atual` fecha. **Duas decisões substituem o texto original da issue** (issue atualizada no GitHub): (1) o turno é por **loja**, não por pessoa — enquanto houver um ABERTO ninguém abre outro (`409 TURNO_JA_ABERTO`); (2) **qualquer cargo abre e fecha**, inclusive CASHIER — é o único módulo da Fase 03 sem bloqueio por cargo, o critério de 403 da RN04 original foi descartado. A trava de unicidade é um **índice único parcial** (`WHERE status = 'ABERTO'`) escrito à mão na migration `20260920002008_turno_caixa_abertura_fechamento`, não só uma checagem no Service: o Service converte o `P2002` em 409 para o caso de duas aberturas simultâneas. Quem abre e quem fecha ficam em colunas separadas (`aberto_por_id`/`fechado_por_id`), então podem ser pessoas diferentes. |
...
- **Pronto, pendente de merge**: #50, #51, #53 e #52 parcial — a Fase 03 inteira, menos o webhook em produção (branch `feature/pagamento-abacatepay-gateway`).
- **Maior lacuna imediata**: com a Fase 03 fechada (PDV, pagamentos e caixa), o próximo bloqueio é a **Fase 04 (NFC-e)** ou a **Fase 05 (assinatura/billing)**. Da Fase 03 sobrou só o webhook da AbacatePay em produção, rastreado na **#79** (validar o `?webhookSecret=`, esclarecer qual chave assina o HMAC, idempotência do reenvio e o split de R$ 0,80) — hoje o Pix funciona por polling.
