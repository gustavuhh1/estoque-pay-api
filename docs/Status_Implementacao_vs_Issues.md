...
- **Pronto, pendente de merge**: #52 parcial (camada de gateway da AbacatePay e validação de assinatura do webhook, branch `feature/pagamento-abacatepay-gateway`).
- **Maior lacuna imediata**: a Fase 02 está inteira em `development`. A Fase 03 começou: o `registrarMovimentacao` do Estoque passou a aceitar transação externa (PR #76), base para a baixa de estoque da venda commitar junto com a `Venda`, e o módulo Caixa (#54) está pronto. Restam **#50 (carrinho/cálculo), #51 (pagamento manual), #52 (Pix/AbacatePay) e #53 (cancelamento)**, nessa ordem de dependência.
